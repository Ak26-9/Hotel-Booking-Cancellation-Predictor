...
# # Gather indexes for training data
# df1_indexes = df1.index.tolist()

# # Shuffle and reset index for training data
# training_data = df1.sample(frac=1, random_state=42).reset_index(drop=True)

# # Create testing dataset from remaining data
...
#         all_acc[name] = accuracy_score(y_test, m.predict(X_test))
#     acc_df = pd.DataFrame.from_dict(all_acc, orient='index', columns=['Accuracy'])
#     st.bar_chart(acc_df)

#     # Correlation Barplot
#     st.subheader("🔍 Top 10 Features by Correlation with Target")
...
#     ax.set_title('Top 10 Correlated Features with is_canceled')
#     st.pyplot(fig)
